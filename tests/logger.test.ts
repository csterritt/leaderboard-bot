...
describe('logger', () => {
  ...
  it('preserves log message content after timestamp', () => {
    ...
    expect(output).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} UTC \[shutdown\] shutting down gracefully\.\.\.$/)
  })
  ...
  it('handles error logging with timestamp', () => {
    ...
    expect(output).toMatch(/^\d{4}-\d{2}-\d{2} \d{2}:\d{2}:\d{2} UTC \[discord\] sendMessage failed$/)
  })
