...
async function dispatch(db: DatabaseType, body: unknown): Promise<Response> {
  return handleInteractionWithVerifier(
...
async function dispatchFetch(db: DatabaseType, body: unknown, mockFetch: typeof fetch): Promise<Response> {
...
describe('/setleaderboardchannel', () => {
  ...
  it('rejects interactions with no member', async () => {
    const res = await dispatch(db, makeInteraction({ member: undefined, data: { name: 'setleaderboardchannel' } }))
    const body = await res.json() as { data: { content: string } }
...
describe('/removeleaderboardchannel', () => {
  ...
  it('rejects interactions with no member', async () => {
    const res = await dispatch(db, makeInteraction({ member: undefined, data: { name: 'removeleaderboardchannel' } }))
    const body = await res.json() as { data: { content: string } }
    expect(body.data.content).toContain('guild')
  })
