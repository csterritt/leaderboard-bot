...
2. **Database** — opens bun:sqlite at `DATABASE_PATH`, enables `PRAGMA journal_mode = WAL` then `PRAGMA foreign_keys = ON` via `db.exec()`, applies `src/db/schema.sql` (idempotent `CREATE TABLE IF NOT EXISTS`). Logs `[startup] opening database` and `[startup] database schema applied`.
